...
<div class="c-layout-sidebar-content ">
				...
				<div class="c-content-title-1">
					<h3 class="c-font-uppercase c-font-bold">Lịch sử quay Bingo</h3>
...
if(isset($_POST['timkiem'])){
$get = $db->query("SELECT * FROM `history_bingo` WHERE `username` = '$uid' ORDER BY id DESC $start $end LIMIT $from,$sotin1trang");
}else{
$get = $db->query("SELECT * FROM `history_bingo` WHERE `username` = '$uid' ORDER BY id DESC LIMIT $from,$sotin1trang");
}
...
<?php } ?>
		...
		<?php 
$tong = $db->query("SELECT * FROM `history_bingo` WHERE `username` = '$uid' ")->rowcount();

if ($tong > $sotin1trang){
echo '<center>' . phantrang('/history/bingo.html?', $from, $tong, $sotin1trang) . '</center>';
} ?>	</div></div></div></div>
