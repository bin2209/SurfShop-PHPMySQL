...
<div class="c-layout-sidebar-content ">
				...
				<div class="c-content-title-1">
					<h3 class="c-font-uppercase c-font-bold">Lịch sử quay thẻ zing</h3>
...
if(isset($_POST['timkiem'])){
$get = $db->query("SELECT * FROM `HK_quaythezing` WHERE `nguoiquay` = '$uid' ORDER BY id DESC $start $end LIMIT $from,$sotin1trang");
}else{
$get = $db->query("SELECT * FROM `HK_quaythezing` WHERE `nguoiquay` = '$uid' ORDER BY id DESC LIMIT $from,$sotin1trang");
}
...
<td><?=$ls['seri'];?></td>
<td><?=$ls['mathe'];?></td>
<td><?=number_format($ls['menhgia']);?> <sup>vnđ</sup></td>
<td><?=$ls['date'];?></td></tr>
<?php } ?>
				</tbody>
				</table>
		<?php 
$tong = $db->query("SELECT * FROM `HK_quaythezing` WHERE `nguoiquay` = '$uid' ")->rowcount();

if ($tong > $sotin1trang){
echo '<center>' . phantrang('/luckywheel/logacc8?', $from, $tong, $sotin1trang) . '</center>';
} ?>	</div></div></div></div>
