...
<script type="text/javascript">
  ...
  $("#sapxep").on('change', function(){
    <?php if(isset($trang)){?>
      if (this.value=='none'){
        // Bỏ sắp xếp
        window.location.href = "<?=$_DOMAIN?>/store<?php if(isset($category)){echo '/'.$category;}?><?php if(isset($type)){echo '/'.$type;}?>?page=<?php if(isset($page))echo $page; else echo 1 ?>";
      }else {
        window.location.href = "<?=$_DOMAIN?>/store<?php if(isset($category)){echo '/'.$category;}?><?php if(isset($type)){echo '/'.$type;}?>?page=<?php if(isset($page))echo $page; else echo 1 ?>&order="+this.value;
      }
    <?php } ?>
  })
</script>
