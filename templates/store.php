...
<style type="text/css">
.store-row-navbar{ border-radius: 12px; margin: 2em; flex-wrap: nowrap; display: flex; -webkit-box-sizing: border-box; box-sizing: border-box; -webkit-box-pack: center; -ms-flex-pack: center; justify-content: center; margin-left: auto; margin-right: auto; width: 70%; background: #101010; } .store-column-narbar{ font-weight: 600; width: 20%; padding: 15px; height: auto; } .store-column-narbar a:hover{ opacity: 1; filter: brightness(200%); cursor: pointer; -webkit-transition: all 0.3s ease-in-out; -o-transition: all 0.3s ease-in-out; transition: all 0.3s ease-in-out; } .store-column-narbar a{ text-decoration: none; opacity: 0.8; color: #fff; } .left-navbar{ overflow: auto; display: block; z-index: 3; position: fixed; float: left; top: 44px; left: 0; height: 100vh; width: 300px; backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); background: rgba(0,0,0,.8); } .left-navbar dl { position: absolute; top: 2%; width: auto; left: 23%; list-style: none; margin-bottom: 10em; } .left-navbar a{ color: #fff; opacity: 0.6; text-decoration: none; font-weight: 600; } .left-navbar a:hover{ opacity: 1; } .left-navbar dt{ margin: 15px 0 0 0; } .left-navbar dd{ width: max-content; text-align: left; margin-left: 28px; margin-top: 2px; } .left-navbar dt a{ font-size: 18px; } .left-navbar dd a{ font-size: 15px; } .close-slidebar{ position: absolute; top: 45%; border: 0px solid; border-radius: 50%; /*background: #323233;*/ left: 89%; } .close-slidebar i{ position: relative; font-size: 40px; top: 4px; color: white; } #open-slidebar,#open-slidebar-mobile i{ position: relative; top: 14px; left: 0px; font-size: 22px; color: white; } .close-slidebar, #open-slidebar,#open-slidebar-mobile i:hover{ cursor: pointer; } #open-slidebar,#open-slidebar-mobile{ z-index: 5; left: 12px; position: fixed; display: none; top: 50%; border: 0px solid; border-radius: 50%; background: #424243; height: 50px; width: 50px; right: 89%;; } .product-row-navbar{ float: left; background: #1f1f1f; } .row{ margin-right: 1px; } /*MOBILE VER STORE VIEW*/ @media only screen and (max-width: 992px) { .close-slidebar{ background: transparent; } .left-navbar dl{ left: 38%; } .close-slidebar i{ top: -44px; } .left-navbar{ width: 100% !important; } .left-navbar{ display: none; } #open-slidebar{ display: none !important; } #open-slidebar-mobile{ display: block; } #Slidebar{ top: 48px; } #open-slidebar-mobile{ display: none; z-index: 5; left: 88%; position: fixed; display: block; top: 50%; border: 0px solid; border-radius: 50%; background: #424243; height: 50px; width: 50px; } .row{ margin-right: auto !important ; } } /*SCROLL*/ #Slidebar::-webkit-scrollbar-track { border-radius: 10px; background-color: rgb(50 50 51); } #Slidebar::-webkit-scrollbar { width: 12px; } #Slidebar::-webkit-scrollbar-thumb { border-radius: 10px; background-color: #555; }
.field-search{ width: 80%; margin-left: auto; margin-right: auto; } .field-search select{ padding: 10px; margin: 10px; border: 1.2px solid #00000063; border-radius: 7px; font-weight: 600; color: #101010; }
</style>
...
$xep = '';
  $xep_status = ''; // rút gọn trạng thái sắp xếp để cho vào function | mục đích page vẫn giữ được sắp xếp
  if(isset($_GET['order'])){
    $order = ($_GET['order']);
    if($order == 'none'){
      $xep = '';
  ...
  // STORE VIEW || BREAKCRUMB
    ...
    // FUNCTION echo content
      ...
      <div class="field-search">
        <select id="sapxep" class="form-control c-square" name="sapxep">
         <option value="none">Mặc định</option>
         <option value="asc"  <?php if(isset($_GET['order'])){if($_GET['order']=='asc'){echo 'selected';}} ?>>Giá: Thấp đến cao</option>
         <option value="desc" <?php if(isset($_GET['order'])){if($_GET['order']=='desc'){echo 'selected';}} ?>>Giá: Cao đến thấp</option>
       </select>
     </div>
     <?php
...
<script type="text/javascript">
  ...
  $("#sapxep").on('change', function(){
    <?php if(isset($trang)){?>
      if (this.value=='none'){
        window.location.href = "<?php echo $link;?>page=<?php echo $trang;?>";
      }else {
        window.location.href = "<?php echo $link;?>page=<?php echo $trang;?>/"+this.value;
      }
    <?php } ?>
  })
</script>
