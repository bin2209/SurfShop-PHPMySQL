...
if(isset($_GET['code'])){
    ...
    if(!isset($token["error"])){
        ...
        if(mysqli_num_rows($get_user) > 0){
            ...
            exit;
            // echo "<script>window.top.location='../login.php?signup-error=Google Account - Enter new password &name=$full_name&email=$email&profile_pic=$profile_pic&google_id=$id';</script>";
        }
    }
    else{
